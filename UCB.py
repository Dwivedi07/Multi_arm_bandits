...
def UCB(delta,N):
    ...
    for j in range(4):
        ...
        mu_hat_UCB[j] =mu_hat[j] + np.sqrt((2*np.log(1/delta))/(pulls[j]))
    ...
    for j in range(N-4):
        ...
        mu_hat_UCB[optimal_arm] =mu_hat[optimal_arm] + np.sqrt((2*np.log(1/delta))/(pulls[optimal_arm]))
...
for j in range(len(deltaD)):
    ...
    Reg = np.zeros(N)
    for i in range(500):
        Reg = (1/(i+1))*(i*Reg + UCB(deltaD[j], N))
    plt.plot(Reg,label=f'UCB with delta ={deltaD[j]}')
